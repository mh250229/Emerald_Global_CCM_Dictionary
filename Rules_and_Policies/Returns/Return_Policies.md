...
![Details Tab](/Images/DetailsTab.png)
...
![Return Policies Restrictions Tab](/Images/ReturnPoliciesRestrictionsTab.png)

|**Field**|**Description**|
|---------|----------|
|**Restrictions**|Defines the conditions by which the return policy is evaluated, in order to identify if the linked action defined in the **Action** area is triggered.|
|#|The number of the restriction. This field is automatically populated by the system|
|Description||A description of the restriction|
|**Rules**|The conditions by which the return policy is evaluated, in order to identify if the linked action defined in the **Action** area is triggered.|
|Attribute|The relevant attribute.<BR>The options are:<BR><BR>**SellableId** - To restrict returns on specific items by their sellable ID. e.g., Item Code 512346000095 cannot be returned.<BR>**Note:** Gift Receipts cannot be issued for items which cannot be returned. If the cashier scans an item which is prohibited to return and then selects the Gift Receipt function, a message prompting that the item is not refundable and a Gift Receipt cannot be issued is displayed.<BR>**ReturnWindow** - To define the number of days up to which an item can be returned. For example, if you enter 30, items can be returned up to 30 days from the date of purchase.<BR>**Note:** This attribute can also be used to specify the number of days a Gift Receipt can be accepted. For example, you can specify that if the Gift Card is valid for 10 days. If a customer presents a Gift Receipt after 10 days, then the action defined in the Action Type field is applied.<BR>For example, manager approval is required when a customer wants to return an item with a Gift Receipt,two weeks after the Gift Receipt was issued.<BR>**IsMemberOfGroup** - To restrict returns on items that are part of a specific group.<BR>**IsMemberOfHierarchy** - To restrict returns on items that are part of a specific hierarchy. E.g. pharmaceutical items cannot be returned.<BR>**IsOpen** - To restrict returns on items that are opened, or items with packaging that is damaged.<BR>**Note:** The IsOpen restriction is not relevant to Return All Transactions. If selected, on saving the Return All policy an error message is prompted.<BR>**LockedForSaleWithReason** - To restrict returns on items that were recalled and prohibited from being sold, and is used to allow returns on recalled items. This is relevant when the selected Reason Code is Recall.<BR>**InStoreVariety** - To restrict returning of items which are not in the store variety in the store in which the items are returned.<BR>For example:<BR>The field InStoreVariety is set to False with the Action Deny.<BR>If the cashier tries to perform a return on an item that is not part of the store's variety in the store in which the item is returned, an error is prompted and the message 'Item is not in the store variety and cannot be returned' is displayed.<BR>**IsOffline** - To restrict returns on items if the central returns server is offline from HQ.<BR>**Note:** The IsOffline restriction is not relevant to Return All Transactions. If selected, on saving the Return All policy an error message is prompted.<BR>**ReasonCode** - To restrict returns on items by a specific Reason Code selected by the cashier, e.g. allow for returns with manager approval if the reason code selected for the return is Customer Regret, or always allow the return if the selected reason code is Defective.<BR>**Reason Code Description** - To restrict returns on items by a specific Reason description selected by the cashier.<BR>**Department** - To restrict returns on items by a specific Department.<BR>**Department Line Amount** - To restrict returns on items by the amount that can be returned in the selected department in a transaction line.<BR>**Total Amount of Department** - To restrict returns on items by the total amount that can be returned in the selected department in a transaction.<BR>**Total Amount of All Departments** - To restrict returns on items by the total amount that can be returned in all department returns.<BR>**Batch Order** – To restrict returns on items in a specific batch order, e.g., items associated to a specific QR Code and Batch Order Label.|
|**Action Types**|The action applied when the defined rule is triggered. The options are:<BR><BR>**Deny** - the return is not allowed.<BR>**Allow With Manager Approval** - the return is allowed only once the manager has approved the return.<BR>**Manager Approval With Forced Exchange** - the returned items can only be exchanged for other items. However, if the manager has overridden the enforcement, a refund is permitted.<BR>**Allow** - the return is permitted.<BR>**Allow With Forced Exchange** - the returned items can only be exchanged for other items. A refund is not permitted.<BR><BR>**Note:** The Manager Approval, Force Exchange, and Force Exchange with Manager Override are not relevant to Return All Transactions. If selected, on saving the Return All policy an error message is prompted.|
|**Scope**|	Defines how often the restriction can be triggered. The options are:<BR>**Request** - indicates that the business rule is always triggered.<BR>**Transaction** - indicates that the business rule is only triggered once per transaction.|
|**Message Name**|The message displayed when the restriction is triggered.|
|**Allowed Roles**|The relevant role with privileges to approve the return when manager approval is required.|